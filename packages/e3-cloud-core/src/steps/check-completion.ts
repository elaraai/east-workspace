...
/**
 * Check completion status of dispatched tasks.
 *
 * This handler:
 * 1. Queries task statuses
...
export async function handleCheckCompletion(executionTracker: ExecutionTracker, event: CheckCompletionEvent): Promise<CheckCompletionResult> {
  ...
  // Get tasks to check: either from dispatchResults or from all in-progress tasks
...
/**
 * Write an event with the next sequence number.
 */
