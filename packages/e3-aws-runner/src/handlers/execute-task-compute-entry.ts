...
import type { TaskExecutionResult, TaskExecutionDeps } from './execute-task-core.js';
import { DynamoDBClient } from '@aws-sdk/client-dynamodb';
import { S3Client } from '@aws-sdk/client-s3';
import { S3ObjectStore } from '@elaraai/e3-aws-storage/s3-object-store';
import { DynamoComputeResultStore, DynamoLogStore } from '@elaraai/e3-aws-storage';
...
async function main(): Promise<void> {
  ...
  const { DynamoDBStateStore } = await import('@elaraai/e3-aws-storage');
  const executions = new DynamoDBStateStore(dynamo, s3, bucketName, tableName);
  const { DynamoRefStore } = await import('@elaraai/e3-aws-storage');
