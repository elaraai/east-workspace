 ...
 * `src/packages/<name>.ts` (one example task pointing its `environment` at the
 * package); a generated `src/packages/index.ts` barrel collects them and the
 * app's `src/index.ts` exports them as the package's dataflow.
 */
...
const RUNTIMES: Record<PackageRuntime, RuntimeConfig> = {
  ...
  python: {
    runtime: "python", destParent: "packages/python", templateName: "python", label: "Python",
  ...
  node: {
    runtime: "node", destParent: "packages/node", templateName: "node", label: "Node",
  ...
  c: {
    runtime: "c", destParent: "packages/native", templateName: "c", label: "C",
...
function writeAppWiring(projectDir: string, names: ProjectNames, members: GeneratedPackage[]): void {
  ...
  const barrel =
    `// Generated by create-e3 — collects one example task per workspace package.\n` +
