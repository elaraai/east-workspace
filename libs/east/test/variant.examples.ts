 ...
 */
import { East, Expr, IntegerType, OptionType, BooleanType, StringType, some, none, example } from "@elaraai/east";
...
export const variantCreate = example({
    ...
    fn: East.function([], OptionType(IntegerType), () => {
        return East.value(true).ifElse(_$ => some(42n), _$ => none);
    }),
    inputs: [],
    returns: some(42n),
});
...
export const variantMatchPartial = example({
    ...
    fn: East.function([], IntegerType, ($) => {
        const v = $.const(none, OptionType(IntegerType));
