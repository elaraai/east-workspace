...
# Usage: curl -fsSL https://raw.githubusercontent.com/elaraai/east-workspace/main/libs/east-claude-plugin/scripts/install.sh | bash
...
install_node_clis() {
    ...
    log_info "Setting up e3 tab completion..."
    e3 completion install 2>/dev/null && log_success "e3 tab completion installed" \
        || log_warn "Could not auto-install e3 completion — run 'e3 completion install' manually"
}
...
print_shell_config() {
    ...
    echo "  e3 repo create ."
    echo "  e3 workspace deploy . dev --from-zip /tmp/pkg.zip"
    echo "  e3 dataflow run . dev"
    echo ""
    echo "Documentation: https://github.com/elaraai/east-workspace"
    echo ""
}
