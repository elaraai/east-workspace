...
The plugin runs preemptive East diagnostics whenever the agent reads an East file (`PostToolUse(Read)` → `hooks/diagnose.js`), edits a python one (`PostToolUse(Edit|Write)`), or writes one through the shell (`PostToolUse(Bash)` → `hooks/diagnose-bash.js`) — and, for the agent's own edits, through the two language servers Claude Code launches: `daemon/lsp.js` for `.ts` / `.tsx` and `daemon/east-py-lsp.js` for `.py` — injecting an `<east-code-review>` block into the agent's context that lists:
...
The rules come from [`@elaraai/east-diagnostics`](../east-diagnostics) and run against a real `ts.Program` (type-aware, not regex). A python file that imports `east` gets the same treatment from the east-py rules (`east.diagnostics`) plus the BUILD's own errors where the project opts in (`[tool.east-py] check = true`). The python language server is a launcher: in a project that depends on east-py (a `pyproject.toml` above the file declares it, or a `.venv` above it has `east-py`, or `EAST_PY_LINT` names the command) it runs the project's own `east-py lsp` — one warm process, rules on every change and the build tier on open and save — and falls back to a cold `east-py lint` per change when that server cannot start; anywhere else it stays a silent null server. The read and edit hooks run `east-py lint --format json` (and `east-py check` where the project opted in) the same way, silently skipped where there is no east-py. `/east:status` reports which of these a project gets. A resident daemon (`daemon/server.js`) holds a warm `LanguageService` per project — started at `SessionStart`, keyed per plugin install so it is shared across sessions and tracks multiple projects at once — so reviews return in well under a second. Reads are reviewed once per distinct file-content (deduped), and vendored/built trees (`node_modules`, `dist`, …) are skipped. The same rule set is available to editors and CI via [`@elaraai/eslint-plugin-east`](../eslint-plugin-east).
