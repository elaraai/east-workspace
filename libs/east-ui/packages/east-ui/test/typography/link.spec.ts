...
describeEast("Link", (test) => {
    ...
    test("creates underline variant link", $ => {
        const link = $.let(Link.Root("Underlined", "/page", { variant: "underline" }));
    ...
    test("creates plain variant link", $ => {
        const link = $.let(Link.Root("Plain", "/page", { variant: "plain" }));
        const style = link.unwrap().unwrap("Link").style.unwrap("some");
    ...
    test("creates link with colorPalette", $ => {
        const link = $.let(Link.Root("Colored", "/page", { colorPalette: "blue" }));
        const style = link.unwrap().unwrap("Link").style.unwrap("some");
    ...
    test("creates link with teal colorPalette", $ => {
        const link = $.let(Link.Root("Teal", "/page", { colorPalette: "teal" }));
        const style = link.unwrap().unwrap("Link").style.unwrap("some");
    ...
    test("creates link with explicit color / hoverColor / visitedColor", $ => {
        const link = $.let(Link.Root("Branded", "/page", {
            color: "#7a3b2e",
            hoverColor: "#552417",
            visitedColor: "#3d1a11",
        }));
    ...
    test("creates navigation link", $ => {
        const link = $.let(Link.Root("Home", "/", { variant: "plain" }));
        const style = link.unwrap().unwrap("Link").style.unwrap("some");
