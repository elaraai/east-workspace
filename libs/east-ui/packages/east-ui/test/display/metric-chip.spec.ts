...
describeEast("MetricChip", (test) => {
    ...
    test("creates a positive MetricChip", $ => {
        const chip = $.let(MetricChip.Root(Text.Root("+12.5%"), "positive"));
    ...
    test("creates a negative solid MetricChip", $ => {
        const chip = $.let(MetricChip.Root(Text.Root("-8.2%"), "negative", { emphasis: "solid" }));
    ...
    test("creates a neutral outline MetricChip with unit", $ => {
        const chip = $.let(MetricChip.Root(Text.Root("42"), "neutral", { emphasis: "outline", unit: "ms" }));
    ...
    test("creates an info MetricChip with explicit colour slots", $ => {
        const chip = $.let(MetricChip.Root(Text.Root("Forecast"), "info", {
            background: "blue.100",
            color: "blue.800",
