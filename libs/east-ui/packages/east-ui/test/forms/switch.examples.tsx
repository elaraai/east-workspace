...
import { Configurator, HStack, Reactive, SegmentGroup, Style, Switch, Text, VStack } from "@elaraai/east-ui";
...
export const switchVariants = example({
    ...
    fn: East.function([], UIComponentType, (_$) => (
        <Reactive>{$ => {
            ...
            // size is an expression — ONE live switch serves every position.
            const sizeSel = $.let(sizes.filter((_$, v) => v.getTag().equal(sKey)).get(0n));
            const preview = $.const(
                <Switch checked={enabled} label="Auto-refresh" size={sizeSel} disabled={disabledOn} onChange={onChange} />,
            );
