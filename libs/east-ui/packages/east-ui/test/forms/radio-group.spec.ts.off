...
describeEast("RadioGroup", (test) => {
    ...
    test("creates radio group with selected value", $ => {
        const r = $.let(RadioGroup.Root("yes", [
            { value: "yes", label: "Yes" },
            { value: "no", label: "No" },
        ]), UIComponentType);
    ...
    test("orientation literal sets variant tag", $ => {
        const r = $.let(RadioGroup.Root("x", [{ value: "x" }], { orientation: "horizontal" }));
    ...
    test("colour overrides round-trip", $ => {
        const r = $.let(RadioGroup.Root("x", [{ value: "x" }], {
            color: "gray.700",
            fillColor: "blue.600",
