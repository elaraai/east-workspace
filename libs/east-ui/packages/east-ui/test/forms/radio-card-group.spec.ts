...
describeEast("RadioCardGroup", (test) => {
    ...
    test("creates card group with selected value", $ => {
        const r = $.let(RadioCardGroup.Root("a", [
            { value: "a", label: "A" },
            { value: "b", label: "B" },
        ]), UIComponentType);
    ...
    test("orientation literal sets variant tag", $ => {
        const r = $.let(RadioCardGroup.Root("x", [{ value: "x", label: "X" }], { orientation: "horizontal" }));
    ...
    test("selectedBorderColor / selectedCardBackground round-trip", $ => {
        const r = $.let(RadioCardGroup.Root("x", [{ value: "x", label: "X" }], {
    ...
    test("onChange callback round-trips on main", $ => {
        ...
        const r = $.let(RadioCardGroup.Root("x", [{ value: "x", label: "X" }], { onChange }));
