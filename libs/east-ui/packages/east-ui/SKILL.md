...
│   │     └─ brush strip is rich by default — the range field's format drives the axis labels, a self-excluding count histogram shows the row distribution; brush={{ axis?, count?, buckets? }} opts down to the bare track
