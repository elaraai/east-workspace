...
│   │     ├─ two card states: the LIST face above + a VIEW state — view={r => <…/>} opens a detail panel on tap (side panel; full-screen sheet on phones; Esc/scrim close, prev/next traversal) · hover={r => <…/>} floating peek on hover-capable pointers · onOpen(key) / onClose() report transitions
