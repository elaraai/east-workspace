...
│   ├─ <Plan axis={Plan.axis({…})} data={rows} series={[Plan.series.span(Row, {…}), …]} /> — the composite temporal canvas: ONE shared time axis (window ÷ resolution = n bucket columns) over heterogeneous KEYED rows — the eight row kinds: span state-runs, bucket allocation lanes, chart measures, heat cells, bucketed table numerals, cards shift chips, event marks, and group strips — sliced, searched, picked and reviewed as one surface. Rows derive from `data` through `series`; a leaf row's key IS its data key (screen order = canonical key order), so a paged source stays addressable by the keys it windows and seeks
│   │   ├─ Props:
...
│   │   │   ├─ slice + affordances (optional) — bound slice chrome (default ["cohort","filter","search","range","resolution","brush","summary"]): the slice's datetime range / resolution IS the window + resolution source of truth (axis seeds the unbound case); `brush` mounts the 32px horizon band (drag its body to slide the window, an edge to resize it, empty track to draw one — every snapped step APPLIES live, so the canvas re-renders honestly mid-gesture and the release commits), `resolution` the WEEK/DAY segment (slice-bound only), `summary` the count line. On a PAGED canvas, narrowing affordances are scope-badged ("loaded rows only") and `search` becomes a KEY SEARCH over the source's `seek` — the jump REBASES residency at the match (windows in between are never fetched)
...
│   │   │   ├─ id + sources + onDrag + canDrop (optional) — DnD TARGET (the shared grammar): Library cards `add` onto span / buckets / events / cards rows only (chart / heat / table render DERIVED values and group is wayfinding — they register no cell), slot = the bucket start instant (Z-less ISO, parses as an East DateTime); drop verdicts resolve LIVE at the pointer, `canDrop` renders the ⊘ stage, and the landing band previews WHERE the card lands
│   │   │   ├─ onSelect / onRunClick / onEventClick / onMarkClick / onChipClick / onCellClick / onGroupToggle (optional) — row selection + per-element clicks; payloads are keys + instants ({ row, run } / { row, event } / { row, mark } / { row, chip } / { row, at }), never indices
...
│   │   ├─ Narrow (below 480px of CONTAINER width — a phone, a splitter pane, a task preview; §10): the same definition reflows to a review tool — Groups · Rows · Measures tabs over ONE slice (cohort / filter / range chips + a WEEK▾ resolution chip; no horizon brush), a slim shared ruler, then cards: Groups = the group grain as hottest-first strip cards (tap opens its rows), Rows = one group's rows with the gutter identity as the card head and the row's plot as the body, Measures = chart rows full-width at expanded density; tap selects, a second tap on an `expand` row drills it in place (~148px; neighbours keep their size), a two-finger horizontal drag pans the window; a paged source shows its resident prefix. Wrap a Plan in `<Box width="360px">` to see it on a desktop page
...
│   │   └─ Factories:
│   │       ├─ Plan.axis({ window?, resolution, resolutions?, now?, format? }) — the shared time-axis declaration
...
│   │       ├─ Plan.tableCells(rawCells) / Plan.tableSeries({ cells, format?, tone?, strong?, rollup? }) — bucketed numerals (multi-series per row, style declared once per position; explicit text/tone overrides via PlanTableCellType values)
│   │       ├─ Plan.layer(chartLayer, { axis?, breach?, series? }) + Plan.fixed("120px") — chart rows consume Chart.Line/Column/Area/Scatter/Band/ref* builder results AS DATA on the shared scale (x accessors must be DateTimeType; Chart.Bar is a build-time error); Plan.layer adds the y-axis side, breach threshold and stack series
