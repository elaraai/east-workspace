...
│   ├─ <ValueTree value={anyEastValue} /> — editable tree of ANY East value, materialized from its STATIC type at authoring time (structs/arrays/string-keyed dicts/options/variants → branches; primitives → typed editable leaves; sets/blobs/vectors/refs/fns → read-only print); omit callbacks for a read-only inspector; style { height, maxHeight } virtualizes rows
│   │     └─ typed path callbacks (host owns the data, re-materialize via <Reactive>): onEdit(path, leaf) leaf edits · onInsert(path) array append / dict add (new key arrives as a trailing `key` step) · onRemove(path) element/entry drop · onTag(path, tag) variant switch + option set/clear ("some"/"none") · ValueTree.zero(T) default element for inserts · ValueTree.Types.{Root,Node,Path,Step,Leaf,Style}
