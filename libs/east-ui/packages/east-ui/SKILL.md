...
│   │     └─ slice={slice} + affordances (Slice chrome rail); sliceEffect={{ excluded: "hide"|"none"|{opacity,desaturate,dot}, emphasis: "halo"|"pulse", frame: true|{fit} }} keeps filtered-OUT items as ghost/desaturate/dot context + emphasises the remainder instead of hiding — feed the FULL set (Slice.partition) and mark item.excluded (e.g. t.matched.not())
