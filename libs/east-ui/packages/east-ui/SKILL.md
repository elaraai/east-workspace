...
│   ├─ <Deck data={rows} card={r => ({ key, title, sublabel, icon, status, tone, facts, filtered })} /> — grouped card collection (display, NOT a drag source — that's Library); groupBy [{ key, label, value, summary }] toolbar + None, layout "grid" (wrap: auto-fill minmax(minCardWidth,1fr) → one phone column) | "list", onCardClick(key), slice (filter/search flow through the slice interface like Table — no bespoke search), style { height, maxHeight, minCardWidth }
│   │     ├─ two card states: the LIST face above + a VIEW state in an anchored POPOVER CARD whose head is inherited from the face — onClick={r => <…/>} supplies the sticky popover's BODY (tap opens; Esc/outside/× closes) · onHover={r => <…/>} the transient peek's body (hover-capable pointers only) · onOpen(key) / onClose() report transitions
│   │     └─ Deck.status(label, tone) pill · tone: "success"|"warning"|"danger"|"info"|"neutral" card accent bar (Deck.Types.Tone for expressions) · facts: Deck.meter(label, value, max, text) / Deck.chips(label, values) / Deck.text(label, text) · render={r => <…/>} full custom card body
...
│   ├─ <ValueTree value={anyEastValue} /> — editable tree of ANY East value, materialized from its STATIC type at authoring time (structs/arrays/dicts/options/variants → branches; primitives → typed editable leaves; sets/blobs/vectors/matrices/refs/fns → read-only summaries; non-string-keyed dicts browsable read-only); omit callbacks for a read-only inspector; style { height, maxHeight } virtualizes rows
│   │     ├─ value handlers (the default — the factory rebuilds the value for you): onUpdate={fn([T], Null)} receives the WHOLE value with the edit applied · at={[ValueTree.at(T, p => p.machines.entry("m1"), fn([SubT], Null))]} scoped subtree handlers (typed probe: struct fields as properties, .item(i), .entry(k), .some(); deepest matching scope wins, unmatched edits bubble to onUpdate)
│   │     └─ RAW path callbacks (escape hatch; override per event): onEdit(path, leaf) · onInsert(path) (array append paths end with an `append` step, dict adds with the new `key`) · onRemove(path) · onTag(path, tag) variant switch + option set/clear ("some"/"none") · ValueTree.zero(T) default element · ValueTree.Types.{Root,Node,Path,Step,Leaf,Style}
...
│   └─ <Route nav={nav} routes={{ route: ($, payload, nav) => <…/> }} /> (#333) — <Pages> generalized to any slot: renders only the active route's body and REMOUNTS it on navigation, but placeable anywhere (a header widget, a sidebar, a drawer body). The body <Pages> and any number of <Route> slots bind the SAME nav handle — lockstep, each remounts its own slot. Use it to swap a STATEFUL component (its own <Reactive>/binds — e.g. a bound queue or table surface) by route; each case is self-contained and rebuilds fresh, so the binds themselves never need to be dynamic. For a non-route key use <Match>
│
...
│   └─ <Match on={bind.read()} cases={{ tag: ($, payload) => <…/> }} /> (#333) — hosting slot over a variant, the component-level twin of variant.match: cases keyed by the variant's case names, exhaustive, each handler gets that case's typed payload. Mounts ONLY the active case and REMOUNTS it on tag change (same-tag payload/data churn re-renders in place) — use it to swap a STATEFUL component (its own <Reactive>/binds) at one slot; a plain variant.match over mounted components keeps the first one mounted (function-blind reconciliation). Pass the READING expression (bind.read()) as `on` — a $.let snapshot freezes the slot. Selecting a plain value by key stays variant.match; a nav-route key is <Route>
│
