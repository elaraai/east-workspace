...
│   │     └─ mode published | edit; days (default Mon–Sun); personWidth (frozen column CSS width, default 150px); state is a PlannerStateType (Roster.Types.State); DnD target — sources={[libraryId]} + onDrag (add/move/remove), onSelect/onAccept/onAddAt
...
│   │     └─ mode published | edit; optional requirements + requirement={r => ({area,shift,required})} coverage (n/required numerals + ⊕ open-slot placeholders, under/over tones); maxVisible +N overflow popover; areaWidth (frozen column, default 150px); canAssign={(person,area,shift) => Boolean} vetoes drops (red outline + ⊘ while dragging; duplicate-person drops likewise); DnD target — sources={[libraryId]} + onDrag (add/move/remove), onSelect/onAccept/onAddAt; zero baked copy (summary/areaHeader are props, toolbar chrome is page composition)
