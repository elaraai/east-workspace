...
│   │   │   ├─ expandRender / expandGutter (optional) — the R2 expand-in-place renders fn(Plan.Types.RowRef) => UIComponent for rows declaring `expand`; the focused row GROWS to hold the render (its gutter cell grows with it; every other data row compresses to a 16px strip, never removed; Esc returns)
...
│   │   │   ├─ slice + affordances (optional) — bound slice chrome (default ["cohort","filter","search","range","resolution","brush","summary"]): the slice's datetime range / resolution IS the window + resolution source of truth (axis seeds the unbound case); `brush` mounts the 32px horizon band (drag = live pan preview, snapped to period edges), `resolution` the WEEK/DAY segment (slice-bound only), `summary` the count line. On a PAGED canvas, narrowing affordances are scope-badged ("loaded rows only") and `search` becomes a KEY SEARCH over the source's `seek` — the jump REBASES residency at the match (windows in between are never fetched)
