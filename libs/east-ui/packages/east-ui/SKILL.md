...
│   │     └─ mode published | edit; optional requirements + requirement={r => ({area,shift,required})} coverage (n/required numerals + ⊕ open-slot placeholders, under/over tones); maxVisible +N overflow popover; DnD target — sources={[libraryId]} + onDrag (add/move/remove; duplicate-person drops are no-ops), onSelect/onAccept/onAddAt; zero baked copy (summary/areaHeader are props, toolbar chrome is page composition)
