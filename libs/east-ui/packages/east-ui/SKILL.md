...
its window. How a consumer bridges the move is its own: a `<Plan>` keeps the
rows it shows until the new snapshot's windows land, and clears a standing
key search.
...
| `Sheet.group(P, "lines", { title, noun: { singular, plural } })` | The word the renderer prints for a group. | `sheetSubRows` |
