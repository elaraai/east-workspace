...
│   ├─ <Deck data={rows} card={r => ({ key, title, sublabel, icon, status, facts, filtered })} /> — grouped card collection (display, NOT a drag source — that's Library); groupBy [{ key, label, value, summary }] toolbar + None, search, layout "grid" (wrap: auto-fill minmax(minCardWidth,1fr) → one phone column) | "list", onCardClick(key), slice, style { height, maxHeight, minCardWidth }
│   │     └─ Deck.status(label, tone) · facts: Deck.meter(label, value, max, text) / Deck.chips(label, values) / Deck.text(label, text) · render={r => <…/>} full custom card body
