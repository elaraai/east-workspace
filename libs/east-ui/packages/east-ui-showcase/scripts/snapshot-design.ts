...
const DESIGN_DIR = path.resolve(SHOWCASE_ROOT, "../../design");
const OUT_DIR = path.join(SHOWCASE_ROOT, "dist-design");
...
async function snapshotBsysSection(
    ...
    const out = path.join(OUT_DIR, `${slug}__bsys.png`);
    try {
        await writeSnapshotPair(section.first(), out, `${slug} · brand-system`);
        return 1;
    } catch (err) {
        console.warn(`[snapshot-design]   skipped ${slug} #brand-system:`, (err as Error).message);
        return 0;
    }
}
...
async function main(): Promise<void> {
    ...
    console.log(`[snapshot-design] static server: ${baseUrl}  (root: ${path.relative(SHOWCASE_ROOT, DESIGN_DIR)})`);
    ...
    try {
        ...
        console.log(`[snapshot-design] done — wrote ${total} images to ${path.relative(SHOWCASE_ROOT, OUT_DIR)}/`);
    } finally {
        await browser?.close();
        await close();
    }
