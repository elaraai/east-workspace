...
test("insertion before the first row emits one draft gesture, applies in order and preserves constructor and hidden fields", async () => {
    ...
    await ui.press(ui.getByRole("button", { name: "Apply changes", exact: true }));
    ...
    ui.refresh(saved);
    expect((ui.getByRole("button", { name: "Apply changes", exact: true }) as HTMLButtonElement).disabled).toBe(true);
...
test("repeated insertion resolves against local identities and Undo restores each exact step", async () => {
    ...
    fireEvent.keyDown(ui.input(), { key: "Escape" }); await ui.flush();
    await ui.press(ui.getByRole("button", { name: "Undo", exact: true }));
    ...
    await ui.press(ui.getByRole("button", { name: "Redo", exact: true }));
