...
import { cellText, formatQuantity, type SheetColumnMeta } from "./model.js";
...
export interface SheetStripProps {
    styles: Styles;
    model: StripModel;
    onPick: (label: string, i: number) => void;
}
                ...
                {model.chips.map((ch) => ch.flat
                    ...
                    : (
                        <Box
                            ...
                            onMouseDown={(e) => { e.preventDefault(); if (ch.pick !== undefined) onPick(ch.title, ch.pick); }}
                        >
                            {ch.label}
                        </Box>
                    ))}
