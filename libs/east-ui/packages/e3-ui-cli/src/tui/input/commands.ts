...
export type ParsedCommand =
    ...
    | { name: 'run'; force: boolean; filter: string | undefined; jobs: number | undefined }
    | { name: 'stop' }
        ...
        case 'run': {
            ...
            for (let i = 0; i < args.length; i++) {
                ...
                if (a === '--force') force = true;
                    ...
                    const raw = a === '--jobs' ? args[++i] : a.slice('--jobs='.length);
                    const n = Number(raw);
                    if (raw === undefined || !Number.isInteger(n) || n < 1) return fail(`--jobs must be a positive integer, got ${raw ?? '(nothing)'}`);
                    jobs = n;
                } else return fail(`unknown /run flag ${a} — /run [--force] [--filter <glob>] [--jobs <n>]`);
            }
            return { ok: true, command: { name, force, filter, jobs } };
        }
        case 'logs': {
            const task = args[0];
...
export interface DescribeContext {
    ...
    running: boolean;
    /** The default jobs budget: runner processes in flight at once. */
    jobs: number;
    /** Pending edits. */
    dirty: number;
}
 ...
 * (`run 6 tasks in main, ignoring the cache · jobs 4`).
 *
...
export function describe(command: ParsedCommand, ctx: DescribeContext): { text: string; keys: string } {
    ...
    switch (command.name) {
        case 'run': {
            ...
            const jobs = command.jobs ?? ctx.jobs;
            return { text: ctx.running ? 'a run is already in progress' : `run ${scope} in ${ws}${cache} · jobs ${jobs}`, keys: ctx.running ? 'esc' : '⏎ run · esc' };
        }
        case 'stop':
