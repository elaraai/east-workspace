...
```
e3-ui [repo] [workspace] [-t <task> | -i <input>] [-j <n>] [--memory <size>] [--no-mouse] [--ascii]
...
- `-j` / `--memory` are the embedded server's budget for a local repository — its cores and memory, as `e3-api-server`'s own flags, defaulting to `E3_JOBS` / `E3_MEMORY`, else the machine's. With a remote repository they are refused: its runs go under its server's budget.
