...
- **Dataflow** — `/run [--force] [--filter] [--jobs]`, `/stop`, live progress in the header pill and the execution panel. (cloud; the extension had it written but never wired)
...
- Confirmations live here: `r` prefills `/run` and the box explains the consequence (`run 6 tasks in main · jobs 4 · ⏎ run · esc`); nothing runs until `⏎`.
...
| `/run [--force] [--filter <glob>] [--jobs <n>]` · `/stop` | dataflow | flags |
...
────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────
 › /run --force_                       run 6 tasks in main, ignoring the cache · jobs 4             ⏎ run · esc         
────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────
 --force  re-run everything    --filter <glob>  only matching tasks    --jobs <n>
```
...
- Frame writer and budgets — `src/tui/ui/render.spec.tsx` mounts the App through Ink's real `render()` into a byte-counting stub terminal (`testing/ink-probe.ts`, 120×40): a selection move writes under 1 KiB against a 5 KiB frame, an idle poll writes nothing. `src/tui/perf.spec.tsx` spawns `testing/perf-probe.ts` under `NODE_ENV=production` (the build the bin runs). Its memory gate runs everywhere and is deterministic: 200 idle polls leave Node's user-timing buffer empty and retain nothing (a bound far above a forced collection's jitter, far below the leak). Its timing gates — the §17 keypress budget on a six-task and a 250-task fixture (`testing/fixtures.ts`), and the 250-task keypress within 2.5× the six-task one — are opt-in (`E3_UI_PERF=1`) and never a CI gate: a shared runner does not measure time reliably, and a gate that fails on a slow moment is a flaky test. Stated at a reference speed, they scale by a fixed calibration workload so another machine reads the same budget; every failure prints the measured numbers; the tests are named `perf:` for `--test-name-pattern`. Two per-keypress counters back the memo gates: `dashboardModel` returns the same object across a selection move, and `treeModelBuilds()` does not advance on one. The frame harness gives ink-testing-library's stdout stub a `rows` after mount: without one Ink falls back to `terminal-size`, a `tput` spawn on every commit (7 ms per keypress in the frame specs, one process per commit in CI).
...
Measured again on 2026-09-25, once the store held every collection as a manifest of segment objects cut by the v2 rule, with `contrib/massive-paging-seed.mjs` and `contrib/massive-paging-probe.mjs`. The same drag over 700,000 wide rows (589 MiB stored, 882 bytes a row, 626 segments) showed the destination 32 ms after the drag ended, with 33 requests at most two in flight, 3 pages retained and a 58 MB heap after collection. Through the app, `/goto 50%` showed the middle in 247 ms, `G` the end in 154 ms and an exact `/find` its key in 563 ms, each with two or three page requests. Over 5,000,000 narrow rows (4,300 segments) the same took 214, 101 and 351 ms.
...
The table's gate is a local one: `E3_UI_PERF=1 make test` measures 16 ms at the reference speed (this box), scaled by a fixed calibration workload on the machine that runs it, with a quarter allowed for noise, plus the 250-task keypress within 2.5× the six-task one. CI does not run it — its first run there measured 17.5 / 28.7 ms on a runner half this speed and failed the absolute number — and runs the deterministic gates instead: the production build through the bin, the bytes a move writes, the model and flatten counters, and the idle heap.
