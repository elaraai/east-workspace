...
| merge | plan, map, reduce(the merge unit task, leading `[mergeIr]` or none, fan-in `MERGE_TREE_FANIN` (32), components), splice | `merge`, or a Set output |
...
**Byte identity.** `merge` must be associative: a unit may fold part of a key's values before the rest, though never out of partition order. Because `merge` writes through the sink's own writer, the merged output does not depend on the tree's shape. With one component, the output is exactly the bytes the ascending sink writes for the folded rows — byte-identical to the same job run as a `streamTask` whose body emits ascending. With several components, each component's blob is those bytes for its own rows, and the output splices them under one header: the same rows, segmented per component.

### The Partition Plan

The plan is an object (`PartitionPlanType`): the partitioned input hashes in wire order, the primary's boundaries (the first segment of each partition), each secondary's split points (segment and element offset, for every partition plus the end), and `slices[input][partition]`, empty until carved. The plan with empty slices is written before anything is carved. When the map step ends — after a failed or cancelled partition too — the plan is written again with whatever was carved, an uncarved slice as `''`, and the `plan` sidecar names it.

A later run of the same logical execution (a retry, or `--force`, which re-runs executions but not the carve) reuses the recorded slices when the recorded plan decodes and plans exactly as this run (equal partitions, boundaries and splits), partition by partition: a partition whose slices were all recorded and still exist is not carved again; any other partition is carved when a worker picks it up.
