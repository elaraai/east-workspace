...
9. **Outcome.**
   - On exit 0, `storeDatasetFile` takes the output file into the store. A collection goes through the store's door (`storeCollection`, in `store-collection.ts`): a stock runner wrote it through the Writer, so its segments are carved out of the file as they stand and named by a manifest, and a `custom` runner's is read a segment at a time and written again. Any other value is hashed by streaming and linked. The `output` ref and a `success` status are then written.
...
- **splice.** The assembly of an earlier step's hashes, in order, through the store's door (`spliceBlobs`, which hands them to `storeCollection`): each collection's segments are carried over by reference, and only the seams between them are re-cut, reading a segment or two either side, so the result is the manifest the Writer writes for the whole value. Set and Dict collections must ascend disjointly in key order, which is checked from their fences, one open at a time. When the step's source yields no hash (every partial empty) the result is the empty collection of the type of the fallback step's first hash.
