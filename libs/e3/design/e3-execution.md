...
7. **Command.** The runner's argv follows from the task's body (`TaskBodyType`):
   ...
   - A `command` body — a `customTask`'s, or a record operation's unit — is its command IR, evaluated over the staged input paths and the output path. A record operation's command is a stock runner's, so `-v` (when verbose) and `--exit-with-parent` are spliced in after `[<bin>, <command>]`. A custom command gets neither.
...
A record's index build (`recordSteps.ts`) fans out on older machinery until records run on the engine. `planPartitions` packs the record's primary segments greedily into slices of about 256 MiB, and `carvePartitionSlices` copies each slice's segments. Each slice runs a synthesized `command` task, the runner's `run` with its emit and stream flags. The partials merge through `mergeComponents`, `planMergeRanges` and the same tree, on each stock runner's `merge` command, and `spliceBlobs` assembles the groups' results through the door. Each unit is an ordinary execution run by the caller's `TaskRunner`, so a rebuild over an unchanged record runs no unit.

The `merge` command sits beside `run` and `exec`: `<runner> merge -p <package>… [--merge <ir> | --union] [--range <blob>] -i <blob>… -o <output>` (`mergeCommandIr` in e3-types builds its argv). It merges sorted Set or Dict blobs of one type: the inputs are read lazily, a segment at a time, through a k-way heap ordered by key and then input index; adjacent equal keys fold in input order — a Dict with the `merge` function, `acc = merge(key, acc, v)`, a Set keeping the first — and the entries are written through the same segment writer as `run`'s emit sink. With `--range`, a blob of `Struct{from: Option<K>, to: Option<K>}` over the inputs' key type, only the keys in `[from, to)` merge: every input's fences are checked to ascend and searched for the segment owning `from` (a fence that does not ascend is refused in the reader's own words), the keys before `from` in that segment are skipped, and the input ends at the first key at or past `to`; an absent bound is open. It holds no spill runs and never reads an input whole. A merge cannot run on the `custom` runtime, which has no `merge` command.
...
A local run has one budget of parallelism: `jobs`, the runner processes e3 keeps in flight at once. Every runner the local runner spawns — a task of the dataflow, a piece or merge unit of a split task, a record operation's unit — holds one slot of a `JobSlots` semaphore from just before its spawn until it has exited, first come first served, and an execution the run aborts while it waits is recorded `cancelled` without a runner ever starting. What is *ready* is decided above it: by the dataflow's loop, whose `concurrency` the CLI sets to the budget, and by the pool of a task run on its own, as wide as the budget. The budget decides what *runs*, so a split task's units queue beside the other tasks of the run instead of multiplying with them.
