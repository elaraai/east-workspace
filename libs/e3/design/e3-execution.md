...
| The run was aborted while the execution waited for the budget, so no runner ever started | `cancelled` | `e3: cancelled: e3 did not start the runner because the run was aborted` |
...
**Determinism.** The pieces, the groups, the ranges and the tree come from the inputs and platform constants — never from the pool width, the budget or timing — and every unit writes through its runner's own writer. So the same task over the same inputs writes the same bytes, one content hash, on every machine, at every `--jobs`, on every runner. `merge` and `combine` must be associative: a unit may fold part of a key's values before the rest, though never out of piece order. Every output is the manifest the Writer writes for the whole value, however many pieces and ranges it was assembled from.
...
An e3 process has one budget of cores and memory (`execution/budget.ts`), and every runner process it spawns takes from it: a task of the dataflow, a piece or merge unit of a split task, a function call, a mutation, an index build. A server's is shared by every run and every call it serves, since the machine is one; each CLI command that runs units makes its own. A runner holds a core and a memory reservation from just before its spawn until it has exited. Requests are granted first come first served, but one whose memory does not fit yet lets later ones that fit pass for ten seconds, and then holds the line until it fits; one that asks for more memory than the whole budget runs alone. An execution the run aborts while it waits is recorded `cancelled` without a runner ever starting.

A reservation is measured in the run, never guessed. A unit of a split task reserves the largest peak resident memory a unit of its stage — the task's pieces, or one level of their merges — has reached in the run, as its runner reported it or as the record of a unit the cache served holds it (`SplitTask.stagePeak`). So each stage runs its first unit alone, while other work goes on, and fans out once it has settled. Nothing is read from earlier runs: a peak that a changed program or input no longer reaches never sizes a unit. A unit with no peak measured before it reserves nothing: a stage's first, a unit of a stage whose runners report no peak, a task run as one unit, a mutation, a function call. The drivers hand a unit its stage's peak as `TaskExecuteOptions.expectedPeakBytes`, which the local runner reserves and a remote backend may size the unit's compute by.

The budget is the local runner's: `LocalTaskRunner` holds it, and `ExecuteOptions.budget` hands it to `taskExecute` for a task run on its own. The dataflow's loop, its steps and state, the `TaskRunner` interface and the API know no budget. The loop keeps `width` tasks and units in flight (`OrchestratorStartOptions.width`, four unless set), which the CLI and the API server set to their budget's cores and a remote backend sets itself. So what is *ready* is decided above the budget — by the loop, and by the pool of a task run on its own, as wide as the budget's cores — and the budget decides what *runs*: a split task's units queue beside the other tasks of the run instead of multiplying with them. The budget is never persisted and never enters an execution's identity.

`-j`/`--jobs` sets the cores and `--memory` the memory, on every command that runs units, on `e3-api-server` and on `e3-ui` for its embedded server; `E3_JOBS` and `E3_MEMORY` stand in for them. The cores default to the CPUs available to the process — `os.availableParallelism()` capped by the tightest cgroup v2 `cpu.max` up the process's hierarchy, as east-c's `east_cpu_count` sizes its pools — and the memory to the tightest cgroup v2 `memory.max`, else physical memory, less a reserve for e3 and the OS: a quarter of it, at most 1 GiB.

Each unit is granted threads from the budget, its `threads`: up to four, and no more than the budget's cores. A runner sizes its own pools to the grant, the frame pool its writers deflate an output's segments on among them, and one thread frames inline. e3's own frame pool, the one the store door frames on, takes two workers, or one on a budget of one core.
...
A dataflow run holds its workspace's `#dataflow` lock, and an ad-hoc `e3 run` holds the repository's `#tasks` lock shared. gc takes `#tasks` exclusively and every workspace's `#dataflow` lock, and refuses while a run holds one. Two attempts at one execution identity each record their own `executionId` directory, and each runs in a scratch directory of its own.
