...
**Runners exit with e3.** On POSIX runners are spawned `detached`, in their own process group, so e3 can stop a whole runner tree — which also means a runner outlives an e3 that dies without warning (a V8 abort, `kill -9`). A stock runner (east-node, east-c, east-py) is therefore given a stdin pipe e3 never writes to and `--exit-with-parent` on its command line (spliced at spawn by `withRunnerLifeline`, after the cache decision, like `-v`): its watcher blocks reading stdin and exits the runner when the read returns end of file, which happens when e3's end of the pipe closes with e3. A `custom` runner keeps an ignored stdin and its argv untouched. (On Windows the job below ends the runner with e3; the lifeline covers an install without the job launcher.)
