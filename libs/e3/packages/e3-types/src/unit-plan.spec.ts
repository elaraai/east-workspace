...
describe('unit plans', () => {
  it('round-trips each stage', () => {
    const plans: UnitPlan[] = [
      { kind: UNIT_PLAN_KIND, task: 'a1', inputs: 'b2', stage: variant('pieces', [['c3', 'd4'], ['e5', 'd4']]) },
      {
  ...
  it('refuses an object of another kind', () => {
    const other = encodeBeast2For(UnitPlanType)({ kind: 'task', task: 'a1', inputs: 'b2', stage: variant('pieces', []) });
