 ...
 * argv a merge execution spawns over sorted partials.
 */
...
describe('mergeCommandIr', () => {
  it('builds a dict merge argv: the merge IR as --merge, every other input a partial', () => {
    assert.deepEqual(
      mergeArgvOf(EAST_C, 'function', ['merge.beast2', 'p0.beast2', 'p1.beast2', 'p2.beast2']),
      ['east-c', 'merge', '-p', 'east-c-std', '--merge', 'merge.beast2',
        '-i', 'p0.beast2', '-i', 'p1.beast2', '-i', 'p2.beast2', '-o', 'out.beast2'],
    );
  });

  it('builds a set union argv: every input a partial', () => {
    assert.deepEqual(
      mergeArgvOf(EAST_C, 'union', ['p0.beast2', 'p1.beast2']),
      ['east-c', 'merge', '-p', 'east-c-std', '--union', '-i', 'p0.beast2', '-i', 'p1.beast2', '-o', 'out.beast2'],
    );
  });

  it('runs the merge command of every stock runtime', () => {
    const py: RunnerValue = variant('east_py', { platforms: ['east-py-std'] });
    assert.deepEqual(mergeArgvOf(py, 'union', ['p0.beast2']).slice(0, 4), ['east-py', 'merge', '-p', 'east-py-std']);
    const node: RunnerValue = variant('east_node', { platforms: [] });
    assert.deepEqual(mergeArgvOf(node, 'function', ['merge.beast2', 'p0.beast2']).slice(0, 2), ['east-node', 'merge']);
  });
