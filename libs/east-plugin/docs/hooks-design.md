...
1. **SessionStart**:
   ...
   # Build hooks first
   cd /path/to/east-claude-plugin && npm run build:hooks

   # Run hook manually
   CLAUDE_PROJECT_DIR=/tmp/test-east CLAUDE_PLUGIN_ROOT=/path/to/east-claude-plugin \
     node /path/to/east-claude-plugin/hooks/dist/session-start.js
   ```
...
2. **UserPromptSubmit**:
   ...
   echo '{"prompt":"How do I create an East function?"}' | \
     CLAUDE_PROJECT_DIR=/tmp/test-east CLAUDE_PLUGIN_ROOT=/path/to/east-claude-plugin \
     node /path/to/east-claude-plugin/hooks/dist/prompt-submit.js
   ```
...
```json
{
  "name": "@elaraai/east-claude-plugin",
  "version": "0.1.0",
...
### Phase 2: Hooks Infrastructure (east-claude-plugin)
