/*  BEAST2 v4 container assembly — the globally-sectioned format.
 *  Public entry points with magic dispatch live in ../full.c.  */
#include "../internal.h"
...
ByteBuffer *east_beast2_v4_encode_full(EastValue *value, EastType *type)
{
    ...
    /* The post-encode error check below reads the shared error slot; drain
     * any stale message a caller left unconsumed so it can't be mistaken for
     * an encode failure. */
