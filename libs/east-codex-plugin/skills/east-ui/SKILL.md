...
│   │       └─ Plan.Types.PatchEvent(R) — what `editing.onPatch` receives for entries of `R` · Plan.Types.Drop ({ from: { library, key }, row: RowId, at: Instant, duplicate }) — what an `edit.create` builds its item from · Plan.Types.Gesture (verdict(ApprovalState) | drop(Drop)) and Plan.Types.RowEdits ({ verdict, drop } — the gestures a row takes) — the editing wire (#880); the session's contracts are the shared `Editing.Types.*` (ChangeSet(R, K), ApplyResult, Readiness)
...
entries. A review verdict and a dropped card are gestures, never callbacks:
    ...
    series={[Plan.series.span(Line, {
        ...
        children: Plan.children(l => l.machines, [
            Plan.series.span(Machine, {
                ...
                edit: {                                      // a dropped card joins `jobs`
                    items: "jobs",
                    create: (drop, m) => ({
...
- **No session, no gesture.** Without `editing` the decision buttons are
  disabled, the foot has no batch, no card lands, and there is no history
  bar. Rerun changes no data, so it stays a callback.
...
- **Plan (subsumes the retired Gantt / Planner / AlignedStack, #571)** —
  ...
  (every verdict and dropped card a draft, #880) and PAGED sources. A Gantt
