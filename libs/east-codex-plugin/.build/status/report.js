// ../east-plugin/dist/status/report.js
...
async function checkPluginStatus(pluginRoot2, cwd2) {
  ...
  checks2.push(await check("Plugin", () => {
    const pkg = JSON.parse(readFileSync2(join3(pluginRoot2, existsSync2(join3(pluginRoot2, ".codex-plugin")) ? ".codex-plugin" : ".claude-plugin", "plugin.json"), "utf8"));
    return { name: "Plugin", status: "ok", detail: `version ${pkg.version ?? "?"} (${pluginRoot2})` };
  }));
  checks2.push(await check("Bundled artifacts", () => {
    const artifacts = existsSync2(join3(pluginRoot2, ".codex-plugin")) ? [...BUNDLED, ".build/hooks/codex-tools.js"] : BUNDLED;
    const missing = artifacts.filter((a) => !existsSync2(join3(pluginRoot2, a)));
  ...
  checks2.push(await check("Hook configuration", () => {
  ...
  checks2.push(await check("East project (cwd)", async () => {
    const { isEast, skills } = await getEastProjectInfo(cwd2);
    return {
  ...
  checks2.push(await check("Diagnostics (PostToolUse daemon)", () => {
    const parts = [];
    let status = "ok";
    const tsOk = resolves(cwd2, "typescript") !== void 0;
    parts.push(tsOk ? "typescript resolvable" : "typescript NOT resolvable");
    if (!tsOk)
      status = "warn";
    const tsconfig = nearestTsconfig(cwd2) !== void 0;
...
function formatStatus(checks2) {
  const anyFail = checks2.some((c) => c.status === "fail");
  const anyWarn = checks2.some((c) => c.status === "warn");
  const header = anyFail ? "East plugin status \u2014 ISSUES" : anyWarn ? "East plugin status \u2014 OK (with notes)" : "East plugin status \u2014 OK";
  const lines = checks2.map((c) => `- ${ICON[c.status]} **${c.name}**: ${c.detail}`);
