...
function daemonEntry() {
  return resolve(dirname2(fileURLToPath2(import.meta.url)), "..", "daemon", "server.js");
}
...
// ../east-plugin/dist/lib/east-rules-context.js
...
function eastRulesContextFor(languages) {
  ...
  const parts = [];
  if (typescript)
    parts.push(EAST_RULES_CONTEXT);
  if (python)
    parts.push(EAST_RULES_CONTEXT_PY);
  return parts.join("\n\n");
}

// ../east-plugin/dist/hooks/session-start.js
async function main() {
