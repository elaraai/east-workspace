...
Every case runs inside ``East.function(...)``, which raises rather than
falling back, so a pass proves the traced path.
"""
...
def test_round_is_half_away_from_zero(x, expected):
    assert East.function(F, IntegerType, lambda v: v.round())(x) == expected
...
def test_floor(x, expected):
    assert East.function(F, IntegerType, lambda v: v.floor())(x) == expected
...
def test_ceil(x, expected):
    assert East.function(F, IntegerType, lambda v: v.ceil())(x) == expected
...
def test_trunc(x, expected):
    assert East.function(F, IntegerType, lambda v: v.trunc())(x) == expected
...
def test_the_math_module_spellings_trace():
    ...
    assert East.function(F, IntegerType, lambda v: math.floor(v))(-3.2) == -4
    assert East.function(F, IntegerType, lambda v: math.ceil(v))(3.2) == 4
    assert East.function(F, IntegerType, lambda v: math.trunc(v))(-3.7) == -3


def test_python_round_names_the_tie_rule_difference():
    with pytest.raises(ExpressionError, match="half away from zero"):
        East.function(F, IntegerType, lambda v: round(v))


def test_to_integer_stays_exact_only():
    assert East.function(F, IntegerType, lambda v: v.to_integer())(3.0) == 3
    from east import EastError
    with pytest.raises(EastError, match="non-integer float"):
        East.function(F, IntegerType, lambda v: v.to_integer())(3.7)


def test_rounding_needs_a_float():
    with pytest.raises(ExpressionError, match="needs a Float"):
        East.function([IntegerType], IntegerType, lambda v: v.round())
    with pytest.raises(ExpressionError, match="needs a Float"):
        East.function([IntegerType], IntegerType, lambda v: v.floor())
...
def test_an_unknown_scalar_method_names_the_real_cause():
    with pytest.raises(ExpressionError,
        ...
        East.function(F, IntegerType, lambda v: v.round_nearest())
