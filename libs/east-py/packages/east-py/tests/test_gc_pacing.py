...
aggregate over 20k rows, ~0.1s, large enough that scheduler jitter is a small
fraction). Pre-fix the last round measured ~12x the first at these sizes and
grew with the round count; post-fix the per-round time is flat. The baseline
is the slower of the first two rounds (warm-up guard) and the bound is 2x on
top of that. The C-level gate (east-c tests/test_gc_pacing.c) asserts the
pacing schedule itself, count-based and timing-free; this test guards the
end-to-end behaviour the issue was filed about.
"""
