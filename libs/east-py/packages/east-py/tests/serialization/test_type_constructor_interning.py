...
emitted twice, diverging from the canonical TS encoding (302 bytes instead of
293 for the case below). These tests pin the interning behaviour.
"""
...
_CANONICAL_SHA256 = "a4d739c985b64f8ad28c8ef8c8308c3a61f38a29d8946769255cac81d789ffaf"
...
def test_repeated_composite_matches_ts_canonical_hash() -> None:
    """The encoding matches the pinned TS-canonical sha256 (293 bytes)."""
    enc = encode_beast2_with_header_for(EastTypeType)
    encoded = enc(_repeated_composite_type())
    assert len(encoded) == 293
