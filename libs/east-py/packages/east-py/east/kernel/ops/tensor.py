...
receiver's kind here — this mixin sits first in ``KernelExpr``'s bases — and
...
class _TensorOps(_ExprBase):
    ...
    def _vector_elem(self, op: str) -> EastType:
        if self.east_type.type != "Vector":
            raise KernelTraceError(f".{op}() on {self.east_type.type} (needs Vector)")
    ...
    def _matrix_numeric(self, op: str) -> EastType:
        if self.east_type.type != "Matrix":
            raise KernelTraceError(f".{op}() on {self.east_type.type} (needs Matrix)")
    ...
    def _scalar_arg(self, op: str, value: Any, t: EastType) -> KernelExpr:
    ...
    def _index_vector(self, op: str, value: Any) -> KernelExpr:
    ...
    def get(self, key: Any = None, col: Any = None) -> KernelExpr:
        """Traced VectorGet / MatrixGet (``get(row, col)``); defers other
        receivers to the collection surface."""
        tag = self.east_type.type
        if tag == "Vector":
            if col is not None:
                raise KernelTraceError("Vector.get() takes one Integer index")
            i = _lift(key)
            if i.east_type.type != "Integer":
                raise KernelTraceError("Vector.get() takes an Integer index")
            elem_t = self.east_type.value
            return self._expr(
                _builtin("VectorGet", elem_t, [elem_t], [self.ir, i.ir]), elem_t)
        if tag == "Matrix":
            if col is None:
                raise KernelTraceError("Matrix.get() takes (row, col) Integer indices")
            r = _lift(key)
            c = _lift(col)
    ...
    def set(self, *args: Any) -> KernelExpr:
        ...
        if tag == "Vector":
            if len(args) != 2:
                raise KernelTraceError("Vector.set() takes (index, value)")
            elem_t = self.east_type.value
            i = _lift(args[0])
            if i.east_type.type != "Integer":
                raise KernelTraceError("Vector.set() index must be an Integer")
        ...
        if tag == "Matrix":
            if len(args) != 3:
                raise KernelTraceError("Matrix.set() takes (row, col, value)")
            elem_t = self.east_type.value
            r = _lift(args[0])
            c = _lift(args[1])
            if r.east_type.type != "Integer" or c.east_type.type != "Integer":
                raise KernelTraceError("Matrix.set() indices must be Integers")
        ...
        if len(args) != 1:
            raise KernelTraceError(f".set() on {tag}")
    ...
    def to_array(self, fn: Any = None, out: EastType | None = None, *,
        ...
        if tag == "Vector":
            if fn is not None or key is not None:
                raise KernelTraceError("Vector.to_array() takes no projection")
            elem_t = self.east_type.value
        ...
        if tag == "Matrix":
            if fn is not None or key is not None:
                raise KernelTraceError("Matrix.to_array() takes no projection")
            elem_t = self.east_type.value
    ...
    def to_matrix(self, rows: Any, cols: Any) -> KernelExpr:
        """Traced VectorToMatrix: reshape into ``rows x cols`` (row-major)."""
        elem_t = self._vector_elem("to_matrix")
        r = _lift(rows)
        c = _lift(cols)
        if r.east_type.type != "Integer" or c.east_type.type != "Integer":
            raise KernelTraceError(".to_matrix() dimensions must be Integers")
    ...
    def scale(self, alpha: Any) -> KernelExpr:
    ...
    def mul(self, other: Any) -> KernelExpr:
    ...
    def add_scalar(self, value: Any) -> KernelExpr:
    ...
    def dot(self, other: Any) -> KernelExpr:
    ...
    def maximum(self, by: Any = None) -> KernelExpr:
        ...
        if self.east_type.type == "Vector":
            if by is not None:
                raise KernelTraceError("Vector.maximum() takes no projection")
    ...
    def mean(self, fn: Any = None) -> KernelExpr:
        ...
        if self.east_type.type == "Vector":
            if fn is not None:
                raise KernelTraceError("Vector.mean() takes no projection")
    ...
    def clamp(self, lo: Any, hi: Any) -> KernelExpr:
    ...
    def gather(self, indices: Any) -> KernelExpr:
    ...
    def scatter_add(self, indices: Any, src: Any) -> KernelExpr:
    ...
    def search_sorted(self, needles: Any) -> KernelExpr:
    ...
    def _mask_compare(self, builtin: str, op: str, other: Any) -> KernelExpr:
    ...
    def compress(self, mask: Any) -> KernelExpr:
    ...
    def vec_mul(self, vector: Any) -> KernelExpr:
