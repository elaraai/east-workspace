...
#
"""The ``KernelExpr`` method surface, one module per domain.

Each module defines a mixin that ``east.kernel.expr`` folds into the concrete
class. A mixin never NAMES ``KernelExpr``: ``expr.py`` imports these modules in
...
class _ExprBase:
    ...
    def _expr(self, ir: Any, east_type: EastType) -> KernelExpr:
