...
Discovers .pyx files under east_py_std/ and compiles them against east-c
(via east-py's build artifacts). Falls back to pure Python if compilation
fails — the import shims in each module handle the fallback.
"""
...
def get_ext_modules():
    ...
    pyx_files = sorted((package_root / "east_py_std").rglob("*.pyx"))

    if not pyx_files:
        return []

    # Get east-c build info from east-py
    try:
    ...
    # Preserve extra_objects/include_dirs through cythonize
    for ext in exts:
