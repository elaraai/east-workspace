...
The namespaced exports (`Console`, `FileSystem`, `Fetch`, `Crypto`, `Time`, `Path`, `Random`, `Parallel`) are the primary API. Use them directly:

```ts
import { describeEast, Assert, Console, FileSystem, NodePlatform } from "@elaraai/east-node-std";

describeEast("My test", (test) => {
    test("reads a file", $ => {
        const content = $.let(FileSystem.readFile("/tmp/test.txt"));
        $(Console.log(content));
    });
